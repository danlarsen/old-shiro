...
namespace Shiro.Interpreter
{
    public class Parser
    {
            ...
            if (m_tokens.Count > 0)
            {
                if (ProcessPound)
                {
                    ...
                    if (m_tokens[0].token == "~")
                    {
                        ...
                        {
                            Error.ReportError("Cannot use Execution Operator (~) with a nonexistant symbol like '" + name + "'");
                        }
                    }
                }
        ...
        protected bool PeekAndDestroy(string expected)
        {
            try
            {
                if (PeekToken().token == expected)
                {
                    PopToken();
                    return true;
                }
